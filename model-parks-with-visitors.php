...
function selectVisitorsByPark($pid) {
    try {
        $conn = get_db_connection();
        $stmt = $conn->prepare("SELECT v.visitor_id, visitor_fname, visitor_age, visit_date, visit_id, t.park_id FROM `visitor` v join visit t on t.visitor_id = v.visitor_id where t.park_id=?");
